 ...
 */
Ext.define('App.Common.calendar.view.MonthDayDetail', {
    ...
    refresh: function() {
        ...
        evts = this.store.queryBy(function(rec) {
            var thisDt = Ext.Date.clearTime(this.date, true).getTime(),
                recStart = Ext.Date.clearTime(rec.data[this.fieldStartDate], true).getTime(),
                startsOnDate = (thisDt == recStart),
                spansDate = false;

            if (!startsOnDate) {
                var recEnd = Ext.Date.clearTime(rec.data[this.fieldEndDate], true).getTime();
        ...
        evts.each(function(evt) {
            ...
            item._renderAsAllDay = item[this.fieldIsAllDay] || App.Common.calendar.util.Date.diffDays(item[this.fieldStartDate], item[this.fieldEndDate]) > 0;
            item.spanLeft = App.Common.calendar.util.Date.diffDays(item[this.fieldStartDate], this.date) > 0;
            item.spanRight = App.Common.calendar.util.Date.diffDays(this.date, item[this.fieldEndDate]) > 0;
