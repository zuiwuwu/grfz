/**
 * @class App.Common.calendar.view.AbstractCalendar
 ...
 */
Ext.define('App.Common.calendar.view.AbstractCalendar', {
    ...
    * {@link App.Common.calendar.EventRecord}) to populate the calendar views with events. Internally this method
    ...
    */
    ...
    initComponent: function () {
        ...
        this.addEvents({
            ...
            * @param {App.Common.calendar.view.AbstractCalendar} this 
            */
            eventsrendered: true,
            /**
            * @event eventclick
            ...
            * @param {App.Common.calendar.view.AbstractCalendar} this
            * @param {App.Common.calendar.EventRecord} rec The {@link App.Common.calendar.EventRecord record} for the event that was clicked on
            ...
            * @param {App.Common.calendar.view.AbstractCalendar} this
            * @param {App.Common.calendar.EventRecord} rec The {@link App.Common.calendar.EventRecord record} for the event that the cursor is over
            ...
            * @param {App.Common.calendar.view.AbstractCalendar} this
            * @param {App.Common.calendar.EventRecord} rec The {@link App.Common.calendar.EventRecord record} for the event that the cursor exited
            ...
            * @param {App.Common.calendar.view.AbstractCalendar} this
            * @param {App.Common.calendar.EventRecord} rec The {@link App.Common.calendar.EventRecord record} for the event that was moved with
            ...
            * @param {App.Common.calendar.view.AbstractCalendar} this
            * @param {App.Common.calendar.EventRecord} rec The {@link App.Common.calendar.EventRecord record} for the event that was deleted
            */
            //eventdelete: true
        });
    },
    ...
    getWeekCount: function () {
        var days = App.Common.calendar.util.Date.diffDays(this.viewStart, this.viewEnd);
            ...
            for (d = 0; d < this.dayCount; d++) {
                if (evtsInView.getCount() > 0) {
                    var evts = evtsInView.filterBy(function (rec) {
                        var startDt = Ext.Date.clearTime(rec.data[this.fieldStartDate], true),
                            startsOnDate = dt.getTime() == startDt.getTime(),
                            spansFromPrevView = (w == 0 && d == 0 && (dt > rec.data[this.fieldStartDate]));
                ...
                dt = App.Common.calendar.util.Date.add(dt, { days: 1 });
            }
        }
    ...
    prepareEventGrid: function (evts, w, d) {
        ...
        evts.each(function (evt) {
            var days = App.Common.calendar.util.Date.diffDays(
            App.Common.calendar.util.Date.max(me.viewStart, evt.data[me.fieldStartDate]),
            App.Common.calendar.util.Date.min(me.viewEnd, evt.data[me.fieldEndDate])) + 1;

            if (days > 1 || App.Common.calendar.util.Date.diffDays(evt.data[me.fieldStartDate], evt.data[me.fieldEndDate]) > 1) {
                ...
                if (evt.data[this.fieldIsAllDay]) {
    ...
    onEventDrop: function (rec, dt) {
        if (App.Common.calendar.util.Date.compare(rec.data[this.fieldStartDate], dt) === 0) {
            // no changes
            return;
        }
        var diff = dt.getTime() - rec.data[this.fieldStartDate].getTime();
        rec.set(this.fieldStartDate, dt);
        rec.set(this.fieldEndDate, App.Common.calendar.util.Date.add(rec.data[this.fieldEndDate], { millis: diff }));
    ...
    onCalendarEndDrag: function (start, end, onComplete) {
        if (start && end) {
            ...
            var o = {};
            o[this.fieldStartDate] = start;
            o[this.fieldEndDate] = end;
    ...
    onUpdate: function (ds, rec, operation) {
        ...
        if (operation == Ext.data.Record.COMMIT) {
            ...
            if (this.enableFx && this.enableUpdateFx) {
                this.doUpdateFx(this.getEventEls(rec.data[this.fieldEventId]), {
                    scope: this
                });
            }
        }
    ...
    onAdd: function (ds, records, index) {
        ...
        if (this.enableFx && this.enableAddFx) {
            this.doAddFx(this.getEventEls(rec.data[this.fieldEventId]), {
                scope: this
            });
        }
    },
    ...
    onRemove: function (ds, rec) {
        ...
        if (this.enableFx && this.enableRemoveFx) {
            this.doRemoveFx(this.getEventEls(rec.data[this.fieldEventId]), {
                remove: true,
                scope: this,
            ...
            this.getEventEls(rec.data[this.fieldEventId]).remove();
            this.refresh();
        }
    },
    ...
    */
    ...
    isEventVisible: function (evt) {
        var data = evt.data || evt,
            ...
            evStart = data[this.fieldStartDate].getTime(),
            evEnd = data[this.fieldEndDate].getTime();
        evEnd = App.Common.calendar.util.Date.add(data[this.fieldEndDate], { seconds: -1 }).getTime();
    ...
    isOverlapping: function (evt1, evt2) {
        ...
        start1 = ev1[this.fieldStartDate].getTime(),
        end1 = App.Common.calendar.util.Date.add(ev1[this.fieldEndDate], { seconds: -1 }).getTime(),
        start2 = ev2[this.fieldStartDate].getTime(),
        end2 = App.Common.calendar.util.Date.add(ev2[this.fieldEndDate], { seconds: -1 }).getTime();
    ...
    */
    setStartDate: function (start, refresh) {
        ...
        var vfilters = [];
        for (var i = 0; i < this.store.filters.length; i++) {
            var filter = this.store.filters.getAt(i);
            if (filter.property == 'STARTTIME') {
            }
            else if (filter.property == 'STOPTIME') {
            }
            else {
                vfilters.push({ property: filter.property,
                    value: filter.value
                });
            }
        }
        vfilters.push({
            property: 'STARTTIME',
            value: Ext.Date.format(this.viewStart, 'Y-m-d')
        });
        vfilters.push({
            property: 'STOPTIME',
            value: Ext.Date.format(this.viewEnd, 'Y-m-d')
        });
        this.store.clearFilter(true);
        this.store.filter(vfilters);
        if (refresh === true) {
            this.refresh();
        }
    ...
    setViewBounds: function (startDate) {
        var start = startDate || this.startDate,
            ...
            Dt = App.Common.calendar.util.Date;
    ...
    * of {@link #App.Common.calendar.EventRecord EventRecord} objects
    */
    sortEventRecordsForDay: function (evts) {
        ...
        evts.sortBy(Ext.bind(function (evtA, evtB) {
            ...
            if (a[this.fieldIsAllDay]) {
                return -1;
            }
            else if (b[this.fieldIsAllDay]) {
                return 1;
            }
            if (this.spansHavePriority) {
                ...
                var diff = App.Common.calendar.util.Date.diffDays;
                if (diff(a[this.fieldStartDate], a[this.fieldEndDate]) > 0) {
                    if (diff(b[this.fieldStartDate], b[this.fieldEndDate]) > 0) {
                        // Both events are multi-day
                        if (a[this.fieldStartDate].getTime() == b[this.fieldStartDate].getTime()) {
                            // If both events start at the same time, sort the one
                            // that ends later (potentially longer span bar) first
                            return b[this.fieldEndDate].getTime() - a[this.fieldEndDate].getTime();
                        }
                        return a[this.fieldStartDate].getTime() - b[this.fieldStartDate].getTime();
                    }
                    return -1;
                }
                else if (diff(b[this.fieldStartDate], b[this.fieldEndDate]) > 0) {
                    return 1;
                }
                return a[this.fieldStartDate].getTime() - b[this.fieldStartDate].getTime();
            }
    ...
    */
    moveNext: function (noRefresh) {
        return this.moveTo(App.Common.calendar.util.Date.add(this.viewEnd, { days: 1 }));
    },
    ...
    */
    movePrev: function (noRefresh) {
        var days = App.Common.calendar.util.Date.diffDays(this.viewStart, this.viewEnd) + 1;
    ...
    */
    moveMonths: function (value, noRefresh) {
        return this.moveTo(App.Common.calendar.util.Date.add(this.startDate, { months: value }), noRefresh);
    },
    ...
    */
    moveWeeks: function (value, noRefresh) {
        return this.moveTo(App.Common.calendar.util.Date.add(this.startDate, { days: value * 7 }), noRefresh);
    },
    ...
    */
    moveDays: function (value, noRefresh) {
        return this.moveTo(App.Common.calendar.util.Date.add(this.startDate, { days: value }), noRefresh);
    },
    ...
    */
    ...
    /**
    * Sets the event store used by the calendar to display {@link App.Common.calendar.EventRecord events}.
    ...
    */
    ...
    getEventRecord: function (id) {
        var idx = this.store.find(this.fieldEventId, id);
    ...
    getTitle: function () {
        return Ext.Date.format(this.startDate, 'Y F');
    },
