 ...
 */
Ext.define('App.Common.calendar.util.WeekEventRenderer', {

    requires: ['App.Common.calendar.util.Date'],
        ...
        render: function (o) {
            ...
            for (; w < weekCount; w++) {
                if (!grid[w] || grid[w].length == 0) {
                    ...
                    dt = App.Common.calendar.util.Date.add(dt, { days: 7 });
                } else {
                    var row,
                        d = 0,
                        wk = grid[w],
                        startOfWeek = Ext.Date.clone(dt),
                        endOfWeek = App.Common.calendar.util.Date.add(startOfWeek, { days: dayCount, millis: -1 });
                        ...
                        if (wk[d]) {
                            ...
                            for (; ev < ct; ev++) {
                                ...
                                if (!evt.isSpan || evt.isSpanStart) {
                                    ...
                                    item._weekIndex = w;
                                    item._renderAsAllDay = item[o.fieldIsAllDay] || evt.isSpanStart;
                                    item.spanLeft = item[o.fieldStartDate].getTime() < startOfWeek.getTime();
                                    item.spanRight = item[o.fieldEndDate].getTime() > endOfWeek.getTime();
                                    ...
                                    var diff = App.Common.calendar.util.Date.diffDays(dt, item[o.fieldEndDate]) + 1,
