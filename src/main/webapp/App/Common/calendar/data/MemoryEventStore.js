 ...
 */
Ext.define('App.Common.calendar.data.MemoryEventStore', {
    extend: 'Ext.data.Store',
    model: 'App.Common.calendar.data.EventModel',
    ...
    proxy: {
        ...
        api: {
            read: '../XJYMng/ListPB',
            create: '../XJYMng/CreatePB',
            update: '../XJYMng/EditPB',
            destroy: '../XJYMng/DelPB'
        },
        reader: {
            type: 'json',
            root: 'rows',
    ...
    constructor: function(config){
        ...
        this.sorters = this.sorters || [{
            property: App.Common.calendar.data.EventMappings.StartDate.name,
            direction: 'ASC'
        }];
        
        this.idProperty = this.idProperty || App.Common.calendar.data.EventMappings.EventId.mapping || 'id';
        this.fields = App.Common.calendar.data.EventModel.prototype.fields.getRange();
    ...
    interceptCreateRecords: function(records, operation, success) {
        if (success) {
            ...
            for (; i < len; i++) {
                records[i].data[App.Common.calendar.data.EventMappings.EventId.name] = records[i].id;
            }
        }
    },
