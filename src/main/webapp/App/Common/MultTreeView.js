...
*/
Ext.define('App.Common.MultTreeView', {
    extend: 'Ext.view.Table',
    alias: 'widget.multtreeview',
    ...
    */
    ...
    uiFields: ['expanded', 'loaded', 'checked', 'expandable', 'leaf', 'icon', 'iconCls', 'loading', 'qtip', 'qtitle', 'mustselchild'],
    ...
    getCurNodeChildChecked: function (curnode) {
        var vCheckedCount = 0;
        var vChildCount = 0;
        var vpart = false;
        curnode.eachChild(function (child) {
            vChildCount++;
            var v = child.get('checked');
            if (v == 'all') {
                vCheckedCount++;
            }
            else if (v == 'part') {
        ...
        if (vChildCount == vCheckedCount) {
            if (vpart)
                return 'part';
            return 'all';
        }
        else if (vCheckedCount > 0
            && vChildCount != vCheckedCount) {
            return 'part';
        }
        else {
            return 'none';
    ...
    setParentChecked: function (curnode, grid) {
        var parent = curnode.parentNode;
        while (parent) {
            var v = grid.getCurNodeChildChecked(parent);
            if (v == 'none') {
                var vmustselchild = parent.raw.mustselchild;
                if (Ext.isBoolean(vmustselchild)
                    && vmustselchild) {
                    if (parent.data.checked != 'none')
                        parent.data.checked = 'none';
                    else
                        break;
                }
                else {
                    if (parent.get('checked') == 'all') {
                        parent.data.checked = 'part';
                    }
                }
            }
            else {
                if (parent.data.checked != v)
                    parent.data.checked = v;
                else
                    break;
            }
    ...
    setChildChecked: function (curnode, checked, grid) {
        curnode.eachChild(function (child) {
            child.data.checked = checked;
            grid.setChildChecked(child, checked, grid);
        });
    },
    ...
    onCheckChange: function (record) {
        ...
        if (Ext.isBoolean(checked)) {
            checked = !checked;
            record.data.checked = checked;
            this.refresh();
            this.fireEvent('checkchange', record, checked);
        }
        else if (Ext.isString(checked)) {
            if (checked == 'all'
            || checked == 'part') {
                checked = 'none';
            }
            ...
            record.data.checked = checked;
            vme.setChildChecked(record, checked, vme);
            vme.setParentChecked(record, vme);
            var vtop = this.el.dom.scrollTop;
            var vleft = this.el.dom.scrollLeft;
            this.refresh();
            this.scrollBy(vleft - this.el.dom.scrollLeft, vtop - this.el.dom.scrollTop);
            this.fireEvent('checkchange', record, checked);
        }
    },
    ...
    setChecked: function (checkedrec) {
        var vme = this;
        var vtemp = {};
        for (var i = 0; i < checkedrec.length; i++) {
            vtemp[checkedrec[i].id] = checkedrec[i];
        }
        this.node.cascadeBy(function (rec) {
            var v = vtemp[rec.get('id')];
            if (typeof v != 'undefined') {
                if (rec.get('checked') != v.checked) {
                    rec.data.checked = v.checked;
                    vme.setParentChecked(rec, vme);
                }
            }
        });
        var vtop = this.el.dom.scrollTop;
        ...
        this.refresh();
        this.scrollBy(vleft - this.el.dom.scrollLeft, vtop - this.el.dom.scrollTop);
        this.fireEvent('checkchange', null, false);
    },

    getChecked: function () {
        var checked = [];
        this.node.cascadeBy(function (rec) {
            var v = rec.get('checked');
            if (Ext.isBoolean(v)
            && v) {
                checked.push(rec);
            }
