//小图按钮定义
Ext.define('App.Common.SPAcorddionBtn', {
    extend: 'Ext.Component',
    alias: 'widget.spacorddionbtn',
    btnCls: Ext.baseCSSPrefix + 'spbtn',
    checked: false,
    enable: true,
    menuActiveCls: '',
    childEls: [
        'innerEl', 'imgEl', 'textEl'
    ],
    renderTpl: [
        '<div id="{id}-innerEl">',
            '<div id="{id}-imgEl" class="{baseCls}-img {imgCls}"></div>',
            '<span id="{id}-textEl" class="{baseCls}-text">{text}</span>',
        '</div>'
    ],
    imgCls: null,
    ...
    onMouseOver: function () {
        ...
        vcls = this.checked ? (vme.imgCls + '-checkedhover') : (vme.imgCls + '-hover');
        if (!vme.imgEl.hasCls(vcls)) {
            vme.imgEl.addCls(vcls);
        }
    },
    onMouseOut: function () {
        ...
        if (vme.imgEl.hasCls(vme.imgCls + '-checkedhover')) {
            vme.imgEl.removeCls(vme.imgCls + '-checkedhover');
        }
        if (vme.imgEl.hasCls(vme.imgCls + '-hover')) {
            vme.imgEl.removeCls(vme.imgCls + '-hover');
        }
    },
    ...
    */
    ...
    setChecked: function (v) {
        ...
        if (vme.checked) {
            ...
            if (vme.imgEl
            &&!vme.imgEl.hasCls(vme.imgCls + '-checked')) {
                vme.imgEl.addCls(vme.imgCls + '-checked');
            }
        }
        else {
            if (this.hasCls(vme.hasImage.checked)) {
            ...
            if (vme.imgEl
            &&vme.imgEl.hasCls(vme.imgCls + '-checked')) {
                vme.imgEl.removeCls(vme.imgCls + '-checked');
            }

            if (vme.imgEl
            &&vme.imgEl.hasCls(vme.imgCls + '-checkedhover')) {
                vme.imgEl.removeCls(vme.imgCls + '-checkedhover');
            }
        }
    },
